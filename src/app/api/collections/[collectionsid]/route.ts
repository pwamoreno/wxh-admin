...
export const GET = async (
  req: NextRequest,
  { params }: { params: { collectionsid: string } }
) => {
  try {
    await connectToDB();

    const collection = await Collection.findById(params.collectionsid).populate(
...
export const DELETE = async (
  ...
  try {
    ...
    await Product.updateMany(
      { collections: params.collectionsid },
      { $pull: { collections: params.collectionsid } }
    );
